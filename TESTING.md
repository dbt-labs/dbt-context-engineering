...
> ⚠️ **Always pass `--full-refresh`.** `ce_ai_run_log` is an *append-only incremental* model, so a
...
ceiling (the whole point of `ce_guard_batch`), rebuild the guarded model with a tiny ceiling — this
is **expected to FAIL**:

```bash
dbt build --project-dir integration_tests/duckdb --profiles-dir integration_tests/duckdb \
  --select ce_guard_pass --vars '{ce_max_batch_rows: 3}'
# EXPECTED: Compilation Error — "ce_guard_batch: batch exceeds ceiling ... rows=10 (max ... =3)"
```
...
and costs real money** (small — the fixtures are ~10 rows; the `ce_generate_text` model is capped
...
ce_model_generate: 'gemini-2.5-flash'        # BigQuery example
ce_embedding_model: 'text-embedding-005'     # BigQuery example
# BigQuery only — cap output + disable Gemini's default "thinking" (billed as output tokens!)
ce_bq_thinking_budget: 0
ce_max_output_tokens: 1024
```
...
dbt build $P --full-refresh --select ce_eval_metrics_bq assert_eval_metrics_bq \
...
dbt build $P --select ce_generate_bq assert_wrappers_nonnull_bq

# 3) The rest of the battery.
dbt build $P --full-refresh

# 4) Version guard needs sequential runs (see §4.1).
dbt build $P --select tag:version_guard --vars '{ce_test_version: v1}'
dbt build $P --select tag:version_guard --vars '{ce_test_version: v2}'
```
...
§5. The **most likely first-run issues** are (a) a wrong `ce_model_*` / embedding var for the account,
...
|---|---|---|
| Chunking | `ce_chunk_utterances`, `ce_chunk_docs`, `ce_chunk_overlap`, `ce_split_docs` | `assert_expected_boundaries`, `assert_soft_cap`, `assert_lineage_exactly_once`, `assert_overlap_boundaries`, `assert_overlap_coverage`, `assert_sentence_split` + `unique`/`not_null` |
| Prompts | `ce_render_prompt_test`, `ce_augment_test` | `assert_render_prompt`, `assert_augment_prompt` (enum injection) |
| Cost guard | `ce_guard_pass` | `not_null` + the §2 trip check |
| Run log | `ce_logged_model`, `ce_ai_run_log` | `assert_run_log` |
| Version guard | `ce_versioned` | `assert_versioned` |
| Retrieval | `ce_search_corpus`, `ce_search_results` | `assert_vector_search` |
| Knowledge base | `ce_kb`, `ce_kb_search`, `source_tickets`, `source_calls` | `assert_knowledge_base`, `assert_kb_search` |
| **Evaluation (P7)** | `ce_eval_predictions`, `ce_eval_metrics` | `assert_eval_metrics`, `assert_grounded`, `assert_conforms`, `assert_conforms_catches`, `ce_grounded` generic test |
| **Output accessors** | `ce_flatten_test` | `assert_flatten`, `assert_flatten_conforms` |

### Each cloud project (`_sf` / `_dbx` / `_bq` suffix) — real warehouse

| Capability | Models (LIVE = real AI call) | Asserted by | Tier |
|---|---|---|---|
| Chunking | `ce_chunk_*`, `ce_chunk_docs_*`, `ce_split_docs_*` | build only (logic proven on duckdb) | deterministic |
| Generate | `ce_generate_*` (LIVE, structured) | `assert_wrappers_nonnull_*` | live |
| Classify | `ce_signals_*` (LIVE, now a scalar label) | `assert_wrappers_nonnull_*`, `assert_signals_conform_*` | live |
| Extract | `ce_extract_*` (LIVE), `ce_extract_flat_*` (ce_field flatten + source text) | `assert_wrappers_nonnull_*`, `assert_extract_conforms_*`, `assert_extract_grounded_*` | live |
| Embed + retrieval | `ce_embeddings_*`, `ce_search_*` (LIVE) | `assert_search_*` | live |
| Knowledge base | `ce_kb_*` (union over the embedded fixture) | `assert_kb_*` | live |
| Run log | populated by `ce_signals_*` post-hook (+ `ce_logged_bq`) → `ce_ai_run_log` | `assert_run_log_*` | live |
| Version guard | `ce_versioned_*` (incremental, no AI; tag `version_guard`) | `assert_versioned_*` via the multi-run CI step (§4.1) | deterministic |
| **Evaluation (P7)** | `ce_eval_metrics_*` (no AI) | `assert_eval_metrics_*`, `assert_grounded_*`, `assert_conforms_catches_*` | deterministic |
| **Output accessors** | `ce_flatten_*` (from `ce_generate_*`), `ce_generate_text_*` (LIVE) | `assert_flatten_conforms_*`, `assert_generate_text_*` | live |

### 4.1 The multi-run version-guard step

`ce_version_guard`'s behavior only appears across *sequential* runs, so the single `dbt build`
...
dbt build --project-dir integration_tests/duckdb --profiles-dir integration_tests/duckdb \
  --select ce_versioned assert_versioned --vars '{ce_test_version: v1}'   # delta path (no bump)
dbt build --project-dir integration_tests/duckdb --profiles-dir integration_tests/duckdb \
  --select ce_versioned assert_versioned --vars '{ce_test_version: v2}'   # bump -> reprocess ALL
# on a cloud project, select by tag so it spans adapters:
dbt build --project-dir integration_tests/bigquery --select tag:version_guard --vars '{ce_test_version: v2}'
```

`assert_versioned` is parameterized by `ce_test_version`, so it proves each state: at v1-again all
...
|---|---|---|
| `assert_eval_metrics(_*)` | 0 rows | `bad_accuracy` → `ce_eval` math or `type_float` cast wrong on this engine. `bad_shape` → the full-outer-join label set changed. Metrics are exact (0.75 / 1.0 / 0.5) — any drift is a real bug, **not** float noise. |
| `assert_grounded(_*)`, `ce_grounded` generic | 0 rows | A returned row = a quote that is **not** a substring of its source after case/whitespace normalization → `ce_contains`/`ce_collapse_ws`/`ce_norm_text` mis-rendered for this dialect (e.g. BigQuery raw-string regex, Snowflake's 4th-arg gotcha). |
| `assert_conforms_catches(_*)` | 0 rows | `ce_schema_enum` failed to resolve the taxonomy, or `IN (...)` list built wrong. |
...
| `assert_flatten_conforms_*` (LIVE) | 0 rows | A row = the flattened `signal` is null or off-taxonomy. Null → `ce_field` path wrong for this engine's output shape (VARIANT `:` / STRUCT `.` / JSON `get_json_object`). Off-taxonomy value → the model ignored the schema (most likely on **BigQuery**, where the enum is prompt-constrained only, not schema-enforced). |
| `assert_generate_text_*` (LIVE) | 0 rows | Empty/null text → `ce_text` didn't extract the payload — watch **BigQuery's** `(...).result` struct access. |
...
| `assert_signals_conform_*` (LIVE) | 0 rows | A row = `ce_classify`'s normalized scalar label is null or off-taxonomy. Null → the per-engine unwrap is wrong (Snowflake `:labels[0]` / BigQuery `.<field>`) — see §6. Off-taxonomy → the model ignored the label set. |
| `assert_extract_conforms_*` (LIVE) | 0 rows | A row = the flattened extract `signal` is null or off-taxonomy. Null → the `ce_field` path is wrong for this engine's extract shape (Snowflake's `:response` envelope is unwrapped in the wrapper; if BigQuery returns null, its STRUCT field access is wrong). |
...
| `assert_kb_*` (LIVE) | 0 rows | `bad_row_count` (≠20) or `missing_source_type` → `ce_knowledge_base`'s union/normalization didn't run as expected on this engine. `null_lineage_or_shape` → a common-shape column (source_id / account_key / embedding / text / source_type) came out null. |
| `assert_run_log_*` (LIVE) | 0 rows | `no_classify_row` → `ce_log_ai_run`'s post-hook INSERT never landed (hook error / ordering). `bad_values` → `row_count`≠10, `est_tokens` null/≤0, or `run_at` null → a cross-engine cast/typing problem in the log INSERT (the classic BigQuery pitfall). |
...
| circuit-breaker trip (§2) | **non-zero exit** | If it exits 0, `ce_guard_batch` failed to raise over the ceiling. |

General cloud red flags: an opaque SQL error at build (not a clean `ce_*` compiler error) usually
...
- **`ce_create_vector_index`** — run-operation only; creates external, billed objects. **Deliberately
...
- **BigQuery Vertex key casing** (`generationConfig`/`thinkingConfig`) — re-confirm on the first live
  BigQuery run.
- **`ce_cost_reconciliation`** — **removed** (2026-07-29); can be re-added later over `ce_ai_run_log`.

**Closed (were gaps in a prior pass):** classify normalized + asserted; extract flattened, conformance
+ evidence-groundedness asserted; `ce_knowledge_base` union exercised on cloud (`assert_kb_*`);
`ce_log_ai_run` INSERT asserted on cloud (`assert_run_log_*`); `ce_version_guard` delta + bump run on
