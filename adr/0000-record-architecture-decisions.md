...
`dbt_context_engineering` makes decisions the code alone does not explain. How AI is exposed (a
row-level SQL function that turns a prompt into a column rather than a custom materialization), where
per-engine divergence is allowed (the dispatched aggregation helpers, and nowhere else), how text is
chunked (deterministic token-bounded packing rather than a semantic chunker), and how output is
normalized are all choices with lasting consequences and real rejected alternatives. There was no
durable, reviewable record of any of them. A reviewer had nothing to weigh in a pull request, and a
future contributor reading the macros had no answer to "why is it done this way?" other than to
reverse-engineer it.
...
[Michael Nygard's](https://adr.github.io/) (Status, Context, Decision, Consequences), extended here
with a plain-language **Concept** opener, a **Reasoning** section that answers "Why ..." questions
from first principles, an optional **Alternatives considered** section, and a closing **Glossary**,
so a record explains itself to a reader new to context engineering.

```text
adr/
  template.md                 copy this to start a record
  0000-record-architecture-decisions.md
  0001-prompts-and-schemas-as-versioned-macros.md
  0002-chunking-as-token-bounded-unit-packing.md
  ...
```
...
Each record carries a `## Status` section whose first line states the status and the date, for
example `Accepted, 2026-07-30.`. Accepted ADRs are immutable. A decision is changed by writing a new
ADR that supersedes the old one and flipping the old one's status to `Superseded by ADR-XXXX`, never
by editing the decision in place. Small factual amendments are noted inline and dated.
...
- **This ADR establishes the practice**; every later record in this directory follows it and the
  shape in `template.md`.
...
- **Michael Nygard format**: the widely used ADR structure (Status, Context, Decision,
  Consequences) this directory follows and extends with Concept, Reasoning, and Glossary.
