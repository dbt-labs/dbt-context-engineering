# 8. Normalizing AI output: scalar classify + `text` / `field`
...
So a column written as `classify(...) as signal` holds a different type on each warehouse, and
...
`embed` already returns a usable vector, and (after this decision) `classify` returns a plain
label, so those need no accessor. The gap is *structured* results from `generate(..., schema)` and
`extract`, whose shape genuinely differs per engine.

## Decision

Normalize the output side:

- **`classify` returns a scalar label string** on all three engines. The per-engine unwrap lives
  ...
  engine object.
- **`text(ai_result)`** — the plain text of an *unstructured* `generate` (BigQuery needs
  `.result`; identity elsewhere).
- **`field(ai_result, field, as_type=none)`** — pulls one field out of a *structured* result and
  casts it, dispatching to the right accessor per engine:

  | engine | how `field` reads a field |
  |---|---|
...
from {{ ref('generated') }}   -- result = generate(..., output_schema=…)
```
...
foreign representation exist at the boundary, but don't let it leak inward. `field` is that
...
- Downstream SQL **never branches on the engine**: flatten once with `field`, then treat the value
...
- Snowflake's `AI_EXTRACT` returns an envelope `{"error":…, "response":{…}}`; the extract wrapper
  unwraps `:response` so its output matches the others and `field('col','signal')` works uniformly
...
- **Semi-structured data** — JSON-like data with named, possibly nested fields, held in warehouse
  types like `VARIANT` / `STRUCT` / `JSON`. Structured output produces it; `field` flattens it
  back to scalars.
...
- **Accessor** — a small helper that reads a value out of a container. `field` is an accessor:
...
- **Adapter dispatch** — dbt's mechanism for picking an engine-specific implementation behind one
  macro name, so `field` reads a VARIANT on Snowflake and a STRUCT on BigQuery from the same call.
- **Output contract** — the promise a wrapper makes about what it returns. Changing `classify` to
