...
1. **Snowflake was the lone platform using a dedicated extract function.** `extract` runs on
...
**We will implement `snowflake__extract` with `AI_COMPLETE` + `response_format`, the same
structured-generate primitive `generate` uses, instead of `AI_EXTRACT`.** This binds the `enum`
structurally and makes all three cloud engines implement extract via structured generate. The result
is the fields object directly (no `:response` envelope), read by the existing `field` path. In the
...
ai_complete(
    model => '{{ model }}',
    prompt => {{ render_prompt(prompt, input_column) }},
...
implement `extract` through their generate primitive. `AI_EXTRACT` was the anomaly. Moving
...
- **All three cloud engines now implement `extract` via structured generate**, and `AI_EXTRACT` is
  no longer used in the package.
- **`extract` on Snowflake now needs a model var** (`model_extract` or `model_generate`),
...
- **Taxonomy / enum**: the fixed set of allowed labels, declared once as the `enum` of a schema
  property (`schema`), single source of truth.
- **Structured-generate primitive**: an engine's generate function driven by an output schema
  ...
  `output_schema`). All three implement `extract`.
