...
The cost guard and run log attach to a model as hooks: `guard_batch` as a pre-hook,
`log_ai_run` as a post-hook. To exercise them in a test you need a model whose build fires those
...
Every tier exercises `log_ai_run` (and `guard_batch`) through a classify model's hooks.
Snowflake and Databricks use their real classify model (`signals_sf` / `signals_dbx`). duckdb
has no AI (`classify` is not implemented there), so it uses a no-AI stand-in, `logged_model`.

bigquery had both: the real `signals_bq` and a no-AI stand-in `logged_bq`. Both fire the same
...
**The no-AI stand-in exists only on duckdb. Remove `logged_bq`.** bigquery exercises
`guard_batch` and `log_ai_run` through its real classify model `signals_bq`, matching
Snowflake and Databricks, and `assert_run_log_bq` depends only on `signals_bq` like the other
cloud tiers.
...
**Why no coverage is lost.** `guard_batch` is still exercised by `signals_bq`'s pre-hook (and by
the dedicated `guard_pass` circuit-breaker), and `log_ai_run` by `signals_bq`'s post-hook.
...
- **No-AI stand-in model**: a model with no AI call whose only purpose is to fire the `guard_batch`
  and `log_ai_run` hooks so they can be tested on a tier that has no real AI model (duckdb:
  `logged_model`).
- **`log_ai_run` / `guard_batch`**: the post-hook that appends a run to `ai_run_log`, and the
