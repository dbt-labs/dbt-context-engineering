{#- Functional-dependency guard for ce_attach_metadata: every metadata column must be constant
    per key in the source relation. If a key carries conflicting values, ce_attach_metadata's
...
select 'ce_fixture_documents' as source, cast(document_id as {{ str_t }}) as key
from {{ ref('ce_fixture_documents') }}
group by document_id
having count(distinct title) > 1 or count(distinct citation_url) > 1
union all
select 'ce_fixture_calls' as source, cast(call_id as {{ str_t }}) as key
from {{ ref('ce_fixture_calls') }}
group by call_id
