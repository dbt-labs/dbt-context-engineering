...
{%- endif -%}
{#- The assertion above is entirely compile-time, so this body only has to be a valid
    zero-row query. BigQuery rejects a WHERE with no FROM, hence the dummy source. -#}
select 1 as ok from (select 1 as _dummy) as _t where 1 = 0
