...
{#- Multi-source union exercise for ce_knowledge_base on this warehouse: two logical sources over
    the same embedded fixture (source_type distinguishes them) -> a 20-row mart carrying both
    source types with lineage. ce_knowledge_base is pure portable SQL (union + common-shape casts,
...
{{ dbt_context_engineering.ce_knowledge_base([
    {'relation': ref('ce_embeddings_dbx'), 'source_type': 'call',
    ...
    {'relation': ref('ce_embeddings_dbx'), 'source_type': 'ticket',
     'source_id': 'utterance_id', 'account_key': 'call_id',
     'text': 'utterance_text', 'embedding': 'embedding',
     'timestamp': "cast('2026-01-01 00:00:00' as " ~ dbt.type_timestamp() ~ ")"}
]) }}
