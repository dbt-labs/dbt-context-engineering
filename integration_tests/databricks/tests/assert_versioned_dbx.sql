...
-- Run by the multi-run CI step with test_version = v1 (delta) then v2 (bump). Fail rows only.
...
from v
where model_version <> '{{ var("test_version", "v1") }}'
union all
select 'bad_count_or_dupes' as issue
from counts
where n_rows <> (select count(*) from {{ ref('fixture_utterances') }})
   or n_ids <> n_rows
