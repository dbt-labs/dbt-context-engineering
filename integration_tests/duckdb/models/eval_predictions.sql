...
{#- Golden-set predictions fixture for Phase 7. Every row is clean: evidence is a verbatim
    ...
    the grounded + conforms_to_schema generic tests attached in schema.yml must PASS. The
    prediction/expected columns feed eval (see eval_metrics). -#}
select
    example_id,
    input_text,
    expected_label,
