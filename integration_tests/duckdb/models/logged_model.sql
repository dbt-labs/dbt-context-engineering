...
{#- Stand-in for an AI model. Its post-hooks append one row to ai_run_log sized from the seed
    (event='started'), then append a SECOND row for the same invocation at event='completed'.
    Validates the run-log + hook-pair end to end on duckdb (the AI call itself is cloud-deferred). -#}
