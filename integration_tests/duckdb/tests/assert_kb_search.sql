...
-- per source (see kb.sql: tickets have it, calls don't) — confirm that optionality survives
-- vector_search's select_columns passthrough too, not just the knowledge-base union. Returns
...
from {{ ref('kb_search') }} having count(*) <> 2
