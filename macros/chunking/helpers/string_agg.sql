{#-
  string_agg(expr, sep, order_expr) -> ordered string concatenation of expr, sequenced by
  order_expr, joined with sep.

  Divergence helper for chunk (spec §5.3 portability note). `sep` is embedded as a literal;
  the default join separator is a real newline character, which is a valid literal on every
  target engine.
-#}

{% macro string_agg(expr, sep, order_expr) -%}
    {{ return(adapter.dispatch('string_agg', 'dbt_context_engineering')(expr, sep, order_expr)) }}
{%- endmacro %}

{#- default: ANSI ordered string_agg (duckdb, postgres, and the local deterministic-test engine). -#}
{% macro default__string_agg(expr, sep, order_expr) -%}
    string_agg({{ expr }}, '{{ sep }}' order by {{ order_expr }})
{%- endmacro %}

{% macro snowflake__string_agg(expr, sep, order_expr) -%}
...
{% macro bigquery__string_agg(expr, sep, order_expr) -%}
...
{% macro databricks__string_agg(expr, sep, order_expr) -%}
