{#-
  ce_conforms_to_schema(relation, column, schema_name, schema_version, property=none,
  allow_null=false) -> a SELECT of the rows whose `column` value is NOT in the enum declared by
  the named ce_schema. Empty result = all rows conform. Catches an LLM that invents a label
  ...
  by name needs the dynamic namespace subscript of ce_schema (D4), which is only available in
  ...
  ce_schema (never a hand-copied list that can drift). Deterministic — no warehouse, no AI.
-#}
{% macro ce_conforms_to_schema(relation, column, schema_name, schema_version,
                               property=none, allow_null=false) -%}
{%- set enum = dbt_context_engineering.ce_schema_enum(
        dbt_context_engineering.ce_schema(schema_name, schema_version), property) -%}
