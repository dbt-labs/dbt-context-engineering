{#-
  ce_eval(relation, prediction_column, expected_column, prompt_version=none) -> a SELECT that
...
-#}
{% macro ce_eval(relation, prediction_column, expected_column, prompt_version=none) -%}
with _base as (
    select
...
select
    {% if prompt_version is not none %}{{ dbt_context_engineering.ce_str_literal(prompt_version) }} as prompt_version,
    {% endif %}metric,
    label,
    value
from _metrics
