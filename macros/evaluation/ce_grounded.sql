{#-
  ce_grounded — generic (schema.yml) test. Asserts each row's evidence/quote actually appears in
...
-#}
{% test ce_grounded(model, column_name, source_text_column,
...
where
    ...
    not (
        {{ dbt_context_engineering.ce_contains(
            dbt_context_engineering.ce_norm_text('_source',   ignore_case, normalize_whitespace),
            dbt_context_engineering.ce_norm_text('_evidence', ignore_case, normalize_whitespace)) }}
    )
