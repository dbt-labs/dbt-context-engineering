{#-
  schema_enum(output_schema, property=none) -> the list of allowed values (the enum) declared
  ...
  Single-sources the label set from the same schema macro the wrappers use (prompts-are-code):
  the eval/conformance tests check against exactly the enum the model was told to produce.
-#}
{% macro schema_enum(output_schema, property=none) -%}
    ...
    {%- if property is not none -%}
        ...
        {%- if pdef is none or not pdef.get('enum') -%}
            {{ exceptions.raise_compiler_error(
                "schema_enum: property '" ~ property ~ "' is absent or has no enum.") }}
        {%- endif -%}
    ...
    {%- endfor -%}
    {%- if ns.found is none -%}
        {{ exceptions.raise_compiler_error("schema_enum: schema has no enum property.") }}
    {%- endif -%}
    {{- return(ns.found) -}}
{%- endmacro %}
