{#-
  bq_model_params(max_output_tokens, thinking_budget) -> a BigQuery `model_params => JSON '...'`
  ...
  The circuit breaker (guard_batch) only guards INPUT tokens, so it cannot catch an output-side
  ...
  the vendor at live-validation time (see generate).
-#}
{% macro bq_model_params(max_output_tokens=none, thinking_budget=none) -%}
    {%- set gen = [] -%}
