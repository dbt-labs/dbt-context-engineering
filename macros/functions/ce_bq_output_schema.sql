{#-
  ce_bq_output_schema(json_schema) -> BigQuery output_schema STRING.
...
-#}
{% macro ce_bq_output_schema(json_schema) -%}
    ...
    {%- endfor -%}
    {%- if (fields | length) == 0 -%}
        {{ exceptions.raise_compiler_error("ce_bq_output_schema: schema has no properties to convert.") }}
    {%- endif -%}
    {{- return(fields | join(', ')) -}}
{%- endmacro %}
