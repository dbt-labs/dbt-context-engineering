{#-
  ce_field(ai_result, field, as_type=none) -> a portable expression that pulls one named field
  out of a STRUCTURED AI result and casts it. This closes the output half of the portability
  promise: ce_generate(..., output_schema)/ce_extract return a different SHAPE per engine
  (Snowflake VARIANT object, Databricks JSON string, BigQuery STRUCT), and this normalizes reading
  a field back out so downstream SQL — and ce_conforms_to_schema — see a plain typed scalar.

  Pair with the wrappers:
    select
      {{ dbt_context_engineering.ce_field('signal_raw', 'signal') }}   as signal,
      {{ dbt_context_engineering.ce_field('signal_raw', 'evidence') }} as evidence
    from {{ ref('signals') }}          -- signal_raw = ce_generate(..., output_schema=ce_schema(...))

  `as_type` defaults to the engine's string type; pass a SQL type (e.g. dbt.type_int()) for others.
  Use ce_text for an UNSTRUCTURED (no output_schema) generation. LIVE-VALIDATION DEFERRED on cloud
  engines; the default (JSON-string) path is validated on duckdb.
-#}
{% macro ce_field(ai_result, field, as_type=none) -%}
    {%- set t = as_type if as_type is not none else dbt.type_string() -%}
    {{ return(adapter.dispatch('ce_field', 'dbt_context_engineering')(ai_result, field, t)) }}
{%- endmacro %}

{#- default = duckdb + baseline for any engine whose result is a JSON string. -#}
{% macro default__ce_field(ai_result, field, as_type) -%}
    cast(json_extract_string({{ ai_result }}, '$.{{ field }}') as {{ as_type }})
{%- endmacro %}

{#- Databricks ai_query structured output returns a JSON STRING. -#}
{% macro databricks__ce_field(ai_result, field, as_type) -%}
...
{% macro snowflake__ce_field(ai_result, field, as_type) -%}
...
{% macro bigquery__ce_field(ai_result, field, as_type) -%}
    cast(({{ ai_result }}).{{ field }} as {{ as_type }})
{%- endmacro %}
