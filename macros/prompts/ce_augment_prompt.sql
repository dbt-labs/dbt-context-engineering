{#-
  ce_augment_prompt(prompt, output_schema) -> the prompt text with an explicit value-constraint
  ...
  ce_bq_output_schema degrades it to plain STRING, so the model never sees the label set and invents
  ...
  Used by the BigQuery wrappers only (bigquery__ce_generate/classify/extract). Snowflake/Databricks
...
-#}
{% macro ce_augment_prompt(prompt, output_schema) -%}
